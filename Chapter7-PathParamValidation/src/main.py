...
#-------------------------
@app.get("/products/{product_id}") 
async def get_product_numeric_ge_le(product_id: Annotated[int, Path(ge=1, le=3)]): 
    for product in PRODUCTS: 
        if product["id"] == product_id: 
            return product 
    return {"error": "Product not found"}
...
#-------------------------
@app.get("/products/{product_id}")
async def get_product_metainfo(product_id: Annotated[int, Path(title="The ID of the product", description="This is product id")]):
    for product in PRODUCTS:
        if product["id"] == product_id:
            return product
    return {"error": "Product not found"}
...
#-------------------------
@app.get("/products/{product_id}")
async def get_product_path_query(
